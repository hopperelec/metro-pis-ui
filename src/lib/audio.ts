...
export const FULL_NAMES_BY_PARTIAL_NAME = Object.entries(AUDIO_FILENAMES)
	.reduce((acc, [category, filenames]) => {
		for (const filename of filenames) {
			...
			else if (filename in UNOFFICIAL_ALIASES) partialName = UNOFFICIAL_ALIASES[filename];
...
export async function concat(fullNames: string[], gapSeconds: number): Promise<string> {
	let wavBuffer: ArrayBuffer;

	const audioContext = new window.AudioContext();
	try {
		// Fetch and decode original buffers
		const buffers = await Promise.all(fullNames.map(async (fullName) => {
