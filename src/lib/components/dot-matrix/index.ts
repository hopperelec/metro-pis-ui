...
export function parseImage(image: HTMLImageElement, expectedCharacters: string, spaceWidth: number) {
    ...
    if (!glyphs[' ']) {
        glyphs[' '] = new Array(spaceWidth).fill(new Array(DOT_MATRIX_HEIGHT).fill(false));
    }
...
export function generateOTF(font: DotMatrixFont, circularDots: boolean, dotSize: number, characterSpacing: number) {
            ...
            for (let fromY = 0; fromY < charData[fromX].length; fromY++) {
                if (charData[fromX][fromY]) {
                    ...
                    if (circularDots) {
                        ...
                        path.moveTo(cx, y1);
                        path.curveTo(cx + dotRadius * KAPPA, y1, x2, cy - dotRadius * KAPPA, x2, cy);
                        path.curveTo(x2, cy + dotRadius * KAPPA, cx + dotRadius * KAPPA, y2, cx, y2);
                        path.curveTo(cx - dotRadius * KAPPA, y2, x1, cy + dotRadius * KAPPA, x1, cy);
                        path.curveTo(x1, cy - dotRadius * KAPPA, cx - dotRadius * KAPPA, y1, cx, y1);
                    } else {
                        path.moveTo(x1, y1);
                        path.lineTo(x2, y1);
                        path.lineTo(x2, y2);
    ...
    const otf = new opentype.Font({
        ...
        ascender: UNITS_PER_EM * .8,
        descender: -UNITS_PER_EM * .2,
        glyphs
    });
...
export function encodeFontBin(characters: DotMatrixFont) {
    ...
    for (let runLengthLimit = 2; runLengthLimit <= maxAllowedRunLength; runLengthLimit++) {
        ...
        const encoding = originalRLE
            ...
            .reduce(
                (acc, digit) => acc * base + BigInt(digit),
                0n
            );
...
export function decodeFontBin(binData: Uint8Array): DotMatrixFont {
    ...
    switch (firstZero) {
        case -1: throw new Error('Dot matrix binary is missing first separator');
        case 0: throw new Error('Dot matrix binary is missing character set');
        case binData.length - 1: throw new Error('Dot matrix binary is missing max run length and RLE');
        case binData.length - 2: throw new Error('Dot matrix binary is missing RLE');
    }
    ...
    let encoding = binData.slice(firstZero+2).reduce(
